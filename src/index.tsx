...
// ============================================================
interface Driver {
  id: string; name: string; phone: string; partner: string;
  status: 'AVAILABLE'|'ON_DUTY'|'OFF'; lat: number; lng: number;
  fcm_token: string; created_at: string;
}
interface Store {
  id: string; name: string; address: string; owner_name: string;
  owner_phone: string; lat: number; lng: number; partner: string;
}
interface Job {
  id: string; store_id: string; store_name: string; store_address: string;
  owner_name: string; owner_phone: string; lat: number; lng: number;
  type: '신규설치'|'AS방문'|'POS교체'|'정기점검';
  subtype: string; driver_id: string|null; driver_name: string;
  partner: string; status: 'UNASSIGNED'|'DISPATCHED'|'DRIVING_TO'|'ARRIVED'|'IN_PROGRESS'|'COMPLETED'|'APPROVED'|'REJECTED'|'CANCELLED';
  scheduled_date: string; scheduled_time: string;
  checklist: CheckItem[]; photos: string[]; signature: boolean;
  note: string; eta: string; delay: number;
  created_at: string; completed_at: string; approved_at: string;
}
interface CheckItem { id: number; label: string; checked: boolean; na: boolean; }
interface Issue {
  id: string; job_id: string; store_name: string; driver_name: string;
  type: string; desc: string; priority: 'P0'|'P1'|'P2';
  status: 'PENDING'|'PROCESSING'|'RESOLVED'; created_at: string;
}
interface History {
  id: string; store_id: string; job_id: string; store_name: string;
  type: string; driver_name: string; status: string;
  completed_at: string; note: string; photos: string[];
}

const DB: {
  drivers: Driver[]; stores: Store[]; jobs: Job[];
  issues: Issue[]; history: History[];
} = {
  drivers: [
    { id:'d1', name:'홍길동', phone:'010-1234-5678', partner:'ABC협력사', status:'ON_DUTY', lat:37.4985, lng:127.0277, fcm_token:'fcm_d1', created_at:'2026-01-01' },
    { id:'d2', name:'이영희', phone:'010-2345-6789', partner:'ABC협력사', status:'ON_DUTY', lat:37.4893, lng:127.0036, fcm_token:'fcm_d2', created_at:'2026-01-01' },
    { id:'d3', name:'김철수', phone:'010-3456-7890', partner:'DEF협력사', status:'ON_DUTY', lat:37.4925, lng:127.0612, fcm_token:'fcm_d3', created_at:'2026-01-01' },
    { id:'d4', name:'박민준', phone:'010-4567-8901', partner:'ABC협력사', status:'AVAILABLE', lat:37.5005, lng:127.0367, fcm_token:'fcm_d4', created_at:'2026-01-01' },
    { id:'d5', name:'최지수', phone:'010-5678-9012', partner:'GHI협력사', status:'AVAILABLE', lat:37.5479, lng:126.9100, fcm_token:'fcm_d5', created_at:'2026-01-01' },
    { id:'d6', name:'정수진', phone:'010-6789-0123', partner:'DEF협력사', status:'OFF',      lat:37.5564, lng:126.9370, fcm_token:'fcm_d6', created_at:'2026-01-01' },
  ],
  stores: [
    { id:'s1', name:'강남 1호점', address:'서울 강남구 테헤란로 123', owner_name:'김점주', owner_phone:'010-1111-2222', lat:37.4985, lng:127.0280, partner:'ABC협력사' },
    { id:'s2', name:'서초 2호점', address:'서울 서초구 서초대로 456', owner_name:'이사장', owner_phone:'010-2222-3333', lat:37.4870, lng:127.0050, partner:'ABC협력사' },
    { id:'s3', name:'대치 3호점', address:'서울 강남구 대치동 789', owner_name:'박대표', owner_phone:'010-3333-4444', lat:37.4924, lng:127.0617, partner:'DEF협력사' },
    { id:'s4', name:'역삼 4호점', address:'서울 강남구 역삼로 101', owner_name:'최사장', owner_phone:'010-4444-5555', lat:37.5000, lng:127.0370, partner:'ABC협력사' },
    { id:'s5', name:'마포 5호점', address:'서울 마포구 마포대로 202', owner_name:'정대표', owner_phone:'010-5555-6666', lat:37.5490, lng:126.9160, partner:'GHI협력사' },
    { id:'s6', name:'홍대 6호점', address:'서울 마포구 홍익로 303', owner_name:'강점주', owner_phone:'010-6666-7777', lat:37.5570, lng:126.9250, partner:'DEF협력사' },
    { id:'s7', name:'신촌 7호점', address:'서울 서대문구 신촌로 404', owner_name:'윤사장', owner_phone:'010-7777-8888', lat:37.5560, lng:126.9380, partner:'DEF협력사' },
    { id:'s8', name:'합정 8호점', address:'서울 마포구 합정동 505', owner_name:'임대표', owner_phone:'010-8888-9999', lat:37.5490, lng:126.9000, partner:'GHI협력사' },
  ],
  jobs: [
    {
      id:'j1', store_id:'s1', store_name:'강남 1호점', store_address:'서울 강남구 테헤란로 123',
      owner_name:'김점주', owner_phone:'010-1111-2222', lat:37.4985, lng:127.0280,
      type:'신규설치', subtype:'POS 단말기', driver_id:'d1', driver_name:'홍길동', partner:'ABC협력사',
      status:'IN_PROGRESS', scheduled_date:'2026-04-08', scheduled_time:'09:00',
      checklist:[
        {id:1,label:'설치 위치 확인',checked:true,na:false},
        {id:2,label:'전원 콘센트 확인',checked:true,na:false},
        {id:3,label:'POS 본체 고정',checked:true,na:false},
        {id:4,label:'모니터 케이블 연결',checked:false,na:false},
        {id:5,label:'결제 단말기 연결',checked:false,na:false},
        {id:6,label:'전원 ON 확인',checked:false,na:false},
        {id:7,label:'초기 설정 완료',checked:false,na:false},
        {id:8,label:'결제 테스트',checked:false,na:false},
      ],
      photos:[], signature:false,
      note:'지하 1층 주차불가. 직원 동반 필요.',
      eta:'14:30', delay:15, created_at:'2026-04-08T08:00:00', completed_at:'', approved_at:''
    },
    {
      id:'j2', store_id:'s2', store_name:'서초 2호점', store_address:'서울 서초구 서초대로 456',
      owner_name:'이사장', owner_phone:'010-2222-3333', lat:37.4870, lng:127.0050,
      type:'AS방문', subtype:'키오스크', driver_id:'d2', driver_name:'이영희', partner:'ABC협력사',
      status:'DRIVING_TO', scheduled_date:'2026-04-08', scheduled_time:'11:00',
      checklist:[
        {id:1,label:'증상 청취',checked:false,na:false},
        {id:2,label:'하드웨어 점검',checked:false,na:false},
        {id:3,label:'소프트웨어 점검',checked:false,na:false},
        {id:4,label:'수리 완료 확인',checked:false,na:false},
        {id:5,label:'테스트 실행',checked:false,na:false},
      ],
      photos:[], signature:false,
      note:'터치스크린 오작동 증상.',
      eta:'11:15', delay:0, created_at:'2026-04-08T09:00:00', completed_at:'', approved_at:''
    },
    {
      id:'j3', store_id:'s3', store_name:'대치 3호점', store_address:'서울 강남구 대치동 789',
      owner_name:'박대표', owner_phone:'010-3333-4444', lat:37.4924, lng:127.0617,
      type:'신규설치', subtype:'POS 단말기', driver_id:'d3', driver_name:'김철수', partner:'DEF협력사',
      status:'DISPATCHED', scheduled_date:'2026-04-08', scheduled_time:'14:00',
      checklist:[
        {id:1,label:'설치 위치 확인',checked:false,na:false},
        {id:2,label:'전원 콘센트 확인',checked:false,na:false},
        {id:3,label:'POS 본체 고정',checked:false,na:false},
        {id:4,label:'모니터 케이블 연결',checked:false,na:false},
        {id:5,label:'결제 단말기 연결',checked:false,na:false},
        {id:6,label:'전원 ON 확인',checked:false,na:false},
        {id:7,label:'초기 설정 완료',checked:false,na:false},
      ...
      photos:[], signature:false, note:'',
      eta:'14:00', delay:0, created_at:'2026-04-08T09:30:00', completed_at:'', approved_at:''
    },
    {
      id:'j4', store_id:'s4', store_name:'역삼 4호점', store_address:'서울 강남구 역삼로 101',
      owner_name:'최사장', owner_phone:'010-4444-5555', lat:37.5000, lng:127.0370,
      type:'POS교체', subtype:'POS 단말기', driver_id:null, driver_name:'', partner:'',
      status:'UNASSIGNED', scheduled_date:'2026-04-08', scheduled_time:'16:00',
      checklist:[
        {id:1,label:'기존 POS 데이터 백업',checked:false,na:false},
        {id:2,label:'기존 장비 철거',checked:false,na:false},
        {id:3,label:'신규 장비 설치',checked:false,na:false},
        {id:4,label:'데이터 이전',checked:false,na:false},
        {id:5,label:'결제 테스트',checked:false,na:false},
        {id:6,label:'영수증 출력 테스트',checked:false,na:false},
      ],
      photos:[], signature:false, note:'구형 POS 1대 교체.',
      eta:'', delay:0, created_at:'2026-04-08T10:00:00', completed_at:'', approved_at:''
    },
    {
      id:'j5', store_id:'s5', store_name:'마포 5호점', store_address:'서울 마포구 마포대로 202',
      owner_name:'정대표', owner_phone:'010-5555-6666', lat:37.5490, lng:126.9160,
      type:'정기점검', subtype:'키오스크', driver_id:null, driver_name:'', partner:'',
      status:'UNASSIGNED', scheduled_date:'2026-04-08', scheduled_time:'10:00',
      checklist:[
        {id:1,label:'외관 점검',checked:false,na:false},
        {id:2,label:'화면 밝기 확인',checked:false,na:false},
        {id:3,label:'결제 모듈 점검',checked:false,na:false},
        {id:4,label:'영수증 프린터 점검',checked:false,na:false},
        {id:5,label:'네트워크 연결 확인',checked:false,na:false},
        {id:6,label:'소프트웨어 업데이트',checked:false,na:false},
      ],
      photos:[], signature:false, note:'6개월 정기점검.',
      eta:'', delay:0, created_at:'2026-04-08T10:30:00', completed_at:'', approved_at:''
    },
    {
      id:'j6', store_id:'s1', store_name:'강남 1호점', store_address:'서울 강남구 테헤란로 123',
      owner_name:'김점주', owner_phone:'010-1111-2222', lat:37.4985, lng:127.0280,
      type:'AS방문', subtype:'POS 단말기', driver_id:'d4', driver_name:'박민준', partner:'ABC협력사',
      status:'COMPLETED', scheduled_date:'2026-04-07', scheduled_time:'10:00',
      checklist:[
        {id:1,label:'증상 청취',checked:true,na:false},
        {id:2,label:'하드웨어 점검',checked:true,na:false},
        {id:3,label:'소프트웨어 점검',checked:true,na:false},
        {id:4,label:'수리 완료 확인',checked:true,na:false},
        {id:5,label:'테스트 실행',checked:true,na:false},
      ],
      photos:['photo1.jpg','photo2.jpg'], signature:true,
      note:'프린터 헤드 교체 완료.',
      eta:'10:00', delay:0, created_at:'2026-04-07T09:00:00', completed_at:'2026-04-07T11:30:00', approved_at:''
    },
  ],
  issues: [
    { id:'i1', job_id:'j1', store_name:'강남 1호점', driver_name:'홍길동', type:'설치 불가', desc:'콘센트 위치 불일치로 연장선 필요', priority:'P0', status:'PENDING', created_at:'2026-04-08T14:23:00' },
    { id:'i2', job_id:'j2', store_name:'서초 2호점', driver_name:'이영희', type:'고객 부재', desc:'점주 연락 두절, 30분 대기 중', priority:'P0', status:'PENDING', created_at:'2026-04-08T13:45:00' },
    { id:'i3', job_id:'j3', store_name:'대치 3호점', driver_name:'김철수', type:'부품 누락', desc:'어댑터 미지참, 물류팀 긴급 요청', priority:'P1', status:'PROCESSING', created_at:'2026-04-08T12:30:00' },
  ],
  history: [
    { id:'h1', store_id:'s1', job_id:'j6', store_name:'강남 1호점', type:'AS방문', driver_name:'박민준', status:'COMPLETED', completed_at:'2026-04-07T11:30:00', note:'프린터 헤드 교체', photos:['photo1.jpg','photo2.jpg'] },
    { id:'h2', store_id:'s1', job_id:'j0', store_name:'강남 1호점', type:'신규설치', driver_name:'홍길동', status:'APPROVED', completed_at:'2026-03-15T14:00:00', note:'POS 신규 설치 완료', photos:['photo3.jpg'] },
  ]
}
...
function uid(prefix: string) { return prefix + '_' + Date.now() + '_' + Math.random().toString(36).slice(2,6) }
function jobById(id: string) { return DB.jobs.find(j=>j.id===id) }
function driverById(id: string) { return DB.drivers.find(d=>d.id===id) }

// ============================================================
// API: DRIVERS
// ============================================================
app.get('/api/drivers', (c) => c.json(DB.drivers))

app.post('/api/drivers', async (c) => {
  const body = await c.req.json()
  const d: Driver = {
    id: uid('d'), name: body.name, phone: body.phone,
    partner: body.partner, status: 'AVAILABLE',
    lat: 37.5665, lng: 126.9780, fcm_token: '', created_at: now()
  }
  DB.drivers.push(d)
  return c.json(d, 201)
})

app.patch('/api/drivers/:id', async (c) => {
  const d = driverById(c.req.param('id'))
...
app.delete('/api/drivers/:id', (c) => {
  const idx = DB.drivers.findIndex(d=>d.id===c.req.param('id'))
...
// API: STORES
// ============================================================
app.get('/api/stores', (c) => c.json(DB.stores))

app.post('/api/stores', async (c) => {
  const body = await c.req.json()
  const s: Store = {
    id: uid('s'), name: body.name, address: body.address,
    owner_name: body.owner_name, owner_phone: body.owner_phone,
    lat: body.lat||37.5665, lng: body.lng||126.9780, partner: body.partner||''
  }
  DB.stores.push(s)
  return c.json(s, 201)
})

app.delete('/api/stores/:id', (c) => {
  const idx = DB.stores.findIndex(s=>s.id===c.req.param('id'))
  if (idx<0) return c.json({error:'not found'},404)
...
// ============================================================
app.get('/api/jobs', (c) => {
  const { date, status, driver_id } = c.req.query()
  let list = [...DB.jobs]
  if (date) list = list.filter(j=>j.scheduled_date===date)
  if (status) list = list.filter(j=>j.status===status)
  if (driver_id) list = list.filter(j=>j.driver_id===driver_id)
  return c.json(list)
})

app.get('/api/jobs/:id', (c) => {
  const j = jobById(c.req.param('id'))
...
app.post('/api/jobs', async (c) => {
  const body = await c.req.json()
  const store = DB.stores.find(s=>s.id===body.store_id)
  const j: Job = {
    id: 'j'+(jobSeq++),
    store_id: body.store_id, store_name: store?.name||body.store_name||'',
    store_address: store?.address||body.store_address||'',
    owner_name: store?.owner_name||body.owner_name||'',
    owner_phone: store?.owner_phone||body.owner_phone||'',
    lat: store?.lat||body.lat||37.5665, lng: store?.lng||body.lng||126.9780,
    type: body.type, subtype: body.subtype||'',
    driver_id: null, driver_name: '', partner: '',
    status: 'UNASSIGNED',
    scheduled_date: body.scheduled_date, scheduled_time: body.scheduled_time||'09:00',
    checklist: getDefaultChecklist(body.type),
    photos: [], signature: false, note: body.note||'',
    eta: '', delay: 0, created_at: now(), completed_at: '', approved_at: ''
  }
  DB.jobs.push(j)
  return c.json(j, 201)
})

function getDefaultChecklist(type: string): CheckItem[] {
  const lists: Record<string,string[]> = {
    '신규설치':['설치 위치 확인','전원 콘센트 확인','POS 본체 고정','모니터 케이블 연결','결제 단말기 연결','전원 ON 확인','초기 설정 완료','결제 테스트'],
    'AS방문':['증상 청취','하드웨어 점검','소프트웨어 점검','수리 완료 확인','테스트 실행'],
    'POS교체':['기존 POS 데이터 백업','기존 장비 철거','신규 장비 설치','데이터 이전','결제 테스트','영수증 출력 테스트'],
    '정기점검':['외관 점검','화면 밝기 확인','결제 모듈 점검','영수증 프린터 점검','네트워크 연결 확인','소프트웨어 업데이트'],
  }
  return (lists[type]||lists['신규설치']).map((label,i)=>({id:i+1,label,checked:false,na:false}))
}
...
app.post('/api/jobs/:id/assign', async (c) => {
  ...
  const d = driverById(driver_id)
  if (!d) return c.json({error:'driver not found'},404)
  j.driver_id = driver_id
  j.driver_name = d.name
  j.partner = d.partner
  j.status = 'DISPATCHED'
  j.eta = j.scheduled_time
  if (d.status==='AVAILABLE') d.status = 'ON_DUTY'
  return c.json(j)
})

// 자동 배정
app.post('/api/jobs/auto-assign', async (c) => {
  const unassigned = DB.jobs.filter(j=>j.status==='UNASSIGNED')
  const available = DB.drivers.filter(d=>d.status!=='OFF')
  let assigned = 0
  for (const job of unassigned) {
    // 가장 가까운 가용 기사 찾기
    let best = available.reduce((prev,cur) => {
      const pd = Math.hypot(prev.lat-job.lat, prev.lng-job.lng)
      const cd = Math.hypot(cur.lat-job.lat, cur.lng-job.lng)
      return cd < pd ? cur : prev
    }, available[0])
    if (!best) continue
    job.driver_id = best.id
    job.driver_name = best.name
    job.partner = best.partner
    job.status = 'DISPATCHED'
    job.eta = job.scheduled_time
    if (best.status==='AVAILABLE') best.status = 'ON_DUTY'
    assigned++
  }
  return c.json({ assigned, total: unassigned.length })
})
...
app.patch('/api/jobs/:id/status', async (c) => {
  ...
  const allowed: Record<string,string[]> = {
    DISPATCHED:['DRIVING_TO','CANCELLED'],
    DRIVING_TO:['ARRIVED','CANCELLED'],
    ARRIVED:['IN_PROGRESS'],
    IN_PROGRESS:['COMPLETED','CANCELLED'],
    COMPLETED:['APPROVED','REJECTED'],
    REJECTED:['IN_PROGRESS'],
  }
  if (!allowed[j.status]?.includes(status)) {
    return c.json({error:`${j.status} → ${status} 전환 불가`},422)
  }
  j.status = status
  if (status==='COMPLETED') {
    j.completed_at = now()
    // 히스토리 추가
    DB.history.push({
      id: uid('h'), store_id: j.store_id, job_id: j.id,
      store_name: j.store_name, type: j.type,
      driver_name: j.driver_name, status: 'COMPLETED',
      completed_at: j.completed_at, note: j.note, photos: j.photos
    })
  }
  if (status==='APPROVED') {
    j.approved_at = now()
    const h = DB.history.find(h=>h.job_id===j.id)
    if (h) h.status = 'APPROVED'
    // 기사 상태 업데이트
    const remaining = DB.jobs.filter(x=>x.driver_id===j.driver_id && ['DISPATCHED','DRIVING_TO','ARRIVED','IN_PROGRESS'].includes(x.status))
    if (remaining.length===0) {
      const d = driverById(j.driver_id||'')
      if (d) d.status = 'AVAILABLE'
    }
  }
  if (status==='CANCELLED') {
    j.driver_id = null; j.driver_name = ''; j.partner = ''; j.status = 'CANCELLED'
  }
  return c.json(j)
})

// 체크리스트 업데이트
app.patch('/api/jobs/:id/checklist', async (c) => {
  const j = jobById(c.req.param('id'))
  if (!j) return c.json({error:'not found'},404)
...
app.post('/api/jobs/:id/photos', async (c) => {
  const j = jobById(c.req.param('id'))
  if (!j) return c.json({error:'not found'},404)
  const { photo_url } = await c.req.json()
  j.photos.push(photo_url || `photo_${Date.now()}.jpg`)
  return c.json(j)
})

// 서명
app.post('/api/jobs/:id/signature', async (c) => {
  const j = jobById(c.req.param('id'))
  if (!j) return c.json({error:'not found'},404)
  j.signature = true
  return c.json(j)
...
// API: ISSUES
// ============================================================
app.get('/api/issues', (c) => c.json(DB.issues))

app.post('/api/issues', async (c) => {
  const body = await c.req.json()
  const j = jobById(body.job_id)
  const issue: Issue = {
    id: uid('i'), job_id: body.job_id,
    store_name: j?.store_name||body.store_name||'',
    driver_name: j?.driver_name||body.driver_name||'',
    type: body.type, desc: body.desc,
    priority: body.priority||'P1', status: 'PENDING', created_at: now()
  }
...
app.patch('/api/issues/:id', async (c) => {
  const issue = DB.issues.find(i=>i.id===c.req.param('id'))
...
app.get('/api/history', (c) => {
  ...
  let list = [...DB.history].sort((a,b)=>b.completed_at.localeCompare(a.completed_at))
  if (store_id) list = list.filter(h=>h.store_id===store_id)
  return c.json(list)
})
...
app.get('/api/dashboard', (c) => {
  ...
  const todayJobs = DB.jobs.filter(j=>j.scheduled_date===today)
  return c.json({
    total: todayJobs.length,
    completed: todayJobs.filter(j=>['COMPLETED','APPROVED'].includes(j.status)).length,
    in_progress: todayJobs.filter(j=>['IN_PROGRESS','ARRIVED','DRIVING_TO'].includes(j.status)).length,
    unassigned: todayJobs.filter(j=>j.status==='UNASSIGNED').length,
    urgent_issues: DB.issues.filter(i=>i.status==='PENDING'&&i.priority==='P0').length,
    drivers_active: DB.drivers.filter(d=>d.status==='ON_DUTY').length,
  })
})
...
app.get('/favicon.svg', (c) => {
  ...
  return c.body(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><rect width="32" height="32" rx="8" fill="#FF6B35"/><text x="16" y="22" text-anchor="middle" font-family="Arial" font-weight="bold" font-size="16" fill="white">FO</text></svg>`)
})
